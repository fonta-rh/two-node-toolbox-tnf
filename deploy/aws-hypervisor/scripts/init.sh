...
ssh -oStrictHostKeyChecking=no $instance_ip 'mkdir -p ~/.ssh'
scp -oStrictHostKeyChecking=no $SSH_PRIVATE_KEY $instance_ip:~/.ssh/id_rsa
scp -oStrictHostKeyChecking=no $SSH_PUBLIC_KEY $instance_ip:~/.ssh/id_rsa.pub

scp -oStrictHostKeyChecking=no "${SCRIPT_DIR}/configure.sh" $instance_ip:~/configure.sh
...
EOF
scp -oStrictHostKeyChecking=no /tmp/profile.env.remote $instance_ip:profile.env
rm /tmp/profile.env.remote

ssh -oStrictHostKeyChecking=no $instance_ip 'sudo chmod +x ~/configure.sh'
...
if [[ -z "${RHSM_ACTIVATION_KEY:-}" ]]; then
    ...
    ssh -oStrictHostKeyChecking=no $instance_ip './configure.sh'
fi
